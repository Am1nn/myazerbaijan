    ...
    <main className={`place-detail-page theme-${theme}`}>
      ...
      <section className="detail-layout">
        ...
        <article className="place-copy"><span>{t.story}</span><p className="lead">{place.description[contentLang]}</p><p>{additionalDescription[contentLang](place.name[contentLang])}</p><h2>{t.facts}</h2><ul>{place.facts[contentLang].map((fact) => <li key={fact}>{fact}</li>)}</ul>
          <div className="route-panel" id="route-options"><div><Navigation /><span><strong>{t.route}</strong><small>{coordinates}</small></span></div><div className="route-links"><a className="route-google" href={`https://www.google.com/maps/dir/?api=1&destination=${coordinates}`} target="_blank" rel="noreferrer"><span className="route-logo"><img src={googleMapsLogo.src} alt="" /></span><span><small>{t.openWith}</small><strong>Google Maps</strong></span><ExternalLink /></a><a className="route-waze" href={`https://www.waze.com/ul?ll=${coordinates}&navigate=yes`} target="_blank" rel="noreferrer"><span className="route-logo"><img src={wazeLogo.src} alt="" /></span><span><small>{t.openWith}</small><strong>Waze</strong></span><ExternalLink /></a></div></div>
        </article>
      </section>
